...
def create_data_model():
    ...
    dist = dist + dist.T
    data["distance_matrix"] = dist.astype(np.int32)
...
def print_solution(manager, routing, solution, elapsed_time):
    ...
    plan_output += f"{elapsed_time} seconds elapsed\n"
    print(plan_output)
...
def main():
    ...
    if solution:
        print_solution(manager, routing, solution, elps)

    matrix = data["distance_matrix"]

    # s = time.time()
    # ans_4 = solve_tsp_simulated_annealing(matrix, max_processing_time=30, perturbation_scheme='ps6')
    # elps = time.time() - s
    # s = time.time()
    # ans_3 = fast_tsp.find_tour(matrix.astype(np.int32))
    # elps = time.time() - s
    # print(ans_4[1], np.sum(ans_3))
